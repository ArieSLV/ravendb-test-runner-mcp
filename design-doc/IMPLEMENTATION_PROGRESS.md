...
|---|---|---|---|---|
| WP_B artifact metadata and attachment thresholds | integrating-agent | Done | WP_B_003 artifact metadata documents and attachment-backed persistence for in-scope v1 artifacts | ArtifactRef metadata and RavenDB attachment persistence landed; attachments-first v1 policy preserved; no build/test execution, MCP host, Web API, UI, or scheduler work |
...
| WP_B_003_artifact_metadata_and_attachment_thresholds | Done | integrating-agent | `3657a64` | `git diff --check` passed; `dotnet build .\RavenDB.TestRunner.McpServer.sln -m:1 -v minimal` succeeded with the `AGENTS.md` isolated .NET SDK 10.0.203 environment; `dotnet test .\tests\RavenDB.TestRunner.McpServer.Storage.RavenEmbedded.Tests\RavenDB.TestRunner.McpServer.Storage.RavenEmbedded.Tests.csproj -m:1 -v minimal --logger "trx;LogFileName=wp-b-003-artifacts.trx"` succeeded with 7 tests discovered/executed | Handoff: `docs/tasks/WP_B/WP_B_003_artifact_metadata_and_attachment_thresholds_HANDOFF.md`; ArtifactRef metadata documents and attachment-backed persistence landed; in-scope v1 artifacts store RavenDB attachments; deferred bulky diagnostics and oversized artifacts record explicit `deferred_external` metadata with no default filesystem route; ENV-001 remains open |
