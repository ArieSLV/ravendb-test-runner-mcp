...
|---|---|---|---|---|
| WP_B artifact metadata and attachment thresholds | integrating-agent | Done | WP_B_003 artifact metadata documents and attachment-backed persistence for in-scope v1 artifacts | Corrective artifact identity validation pass accepted; attachments-first v1 policy preserved; no build/test execution, MCP host, Web API, UI, or scheduler work |
| WP_C v7.2 semantics plugin | worker/Ampere | Done | WP_C_005 v7.2 semantics plugin baseline | Handoff accepted; worker used model `gpt-5.5` with `reasoning_effort=xhigh`; integrator validation accepted 8 semantics checks; no external docs used |
| WP_D build fingerprint and reuse engine | integrating-agent | Done | WP_D_003 deterministic fingerprint, reuse, readiness, and invalidation policy/domain work | Deterministic fingerprint/reuse/readiness policy landed; corrective expert-skip ownership proof and reason-code normalization passes accepted; no actual build execution, process spawning, scheduler, MCP host, Web API, UI, or test execution subsystem work |
