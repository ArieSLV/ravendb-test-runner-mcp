...
|---|---|---|---|---|
| WP_B embedded bootstrap and database init | integrating-agent | Done | WP_B_001 corrective pass for embedded bootstrap lifecycle control | Process-wide EmbeddedServer singleton behavior now has explicit matching/rejecting lifecycle control; attachments-first policy remains unchanged |
| WP_C workspace and repo line detection | integrating-agent | Done | WP_C_001 corrective pass for deterministic bounded scanning | Directory/file traversal is deterministic before caps are applied; truncated close-score detection now marks ambiguity explicitly |
