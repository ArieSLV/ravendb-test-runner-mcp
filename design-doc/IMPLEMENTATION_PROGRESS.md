...
| WP_C v7.1 semantics plugin | worker/Parfit | In Progress | WP_C_004 v7.1 semantics plugin baseline | Single delegated worker spawned with model `gpt-5.5` and `reasoning_effort=xhigh`; no ledger edits delegated |
...
| WP_C_004_v71_semantics_plugin | In Progress | worker/Parfit |  | Pending | Delegated worker spawned with model `gpt-5.5` and `reasoning_effort=xhigh`; no `IMPLEMENTATION_PROGRESS.md` edits delegated |
